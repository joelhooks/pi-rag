...
export default function piRagExtension(pi: Pi) {
  ...
  pi.registerTool({
    ...
    async execute(_id: string, params: { query: string; limit?: number; corpusTree?: boolean }) {
  ...
  pi.registerTool({
    ...
    async execute(_id: string, params: { sessionId: string; query: string; fanout?: number; maxDepth?: number }) { return text(await service.searchTree(params.sessionId, params.query, { fanout: params.fanout, maxDepth: params.maxDepth })); },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id: string, params: { sessionId: string; nodeId: string; usePi?: boolean }) { return text(await service.summarizeNode(params.sessionId, params.nodeId, { usePi: params.usePi })); },
  });
}
