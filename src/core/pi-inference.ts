...
export class HeuristicSummarizer implements Summarizer {
  async summarizeNode(messages: SessionMessage[], node: RagNode): Promise<NodeSummary> {
...
export class PiCliSummarizer implements Summarizer {
  constructor(private command = process.env.PI_RAG_PI_COMMAND || "pi") {}

  async summarizeNode(messages: SessionMessage[], node: RagNode, doc: SessionDocument): Promise<NodeSummary> {
...
function runPi(command: string, prompt: string): Promise<string> {
...
function extractJson(text: string): any {
  ...
  if (start < 0 || end < start) throw new Error(`no JSON object in pi output: ${text.slice(0, 500)}`);
