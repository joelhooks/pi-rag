...
function roleMix(messages) {
    return messages.reduce((acc, m) => { acc[m.role] = (acc[m.role] || 0) + 1; return acc; }, {});
}
function node(id, messages, start, end, trace, children) {
    return { id, title: titleFrom(messages, id), summary: summarize(messages), start, end, roleMix: roleMix(messages), children, trace };
}
function shouldBreak(prev, curr, indexInSegment, windowSize) {
    if (indexInSegment >= windowSize)
        return true;
    if (prev.createdAt && curr.createdAt) {
        const delta = Date.parse(curr.createdAt) - Date.parse(prev.createdAt);
        if (Number.isFinite(delta) && delta > 1000 * 60 * 45)
            return true;
    }
    const currText = curr.content.toLowerCase();
    if (/^(new topic|separate|anyway|switching|next|question:)/.test(currText.trim()))
        return true;
    return false;
}
export function buildStructure(doc, opts = {}) {
...
export function findNode(structure, nodeId) {
...
export function getBoundedContent(doc, structure, input) {
    ...
    if (input.nodeId) {
        ...
        trace.push(`range selected from node ${input.nodeId}: ${start}-${end}`);
    }
    if (start == null || end == null)
        throw new Error("bounded retrieval requires nodeId or explicit start/end");
    if (start < 0 || end < start || end >= doc.messages.length)
        throw new Error(`invalid range ${start}-${end} for ${doc.messages.length} messages`);
    const count = end - start + 1;
    if (!input.allowLarge && count > MAX_INLINE_MESSAGES)
        throw new Error(`refusing unbounded/large dump (${count} messages). Use a tighter node/range or allowLarge=true.`);
