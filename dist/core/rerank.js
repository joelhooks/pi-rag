...
const DEFAULT_WEIGHTS = { bm25: 1.0, typesense: 0.35, recency: 0.18, exact: 0.35, project: 0.25 };
const K1 = 1.2;
const B = 0.75;
...
export function bm25Score(query, docs) {
    ...
    for (const qt of qTerms)
        df.set(qt, tokenized.filter((d) => d.includes(qt)).length);
...
function projectScore(hints, text) {
    if (!hints?.length)
        return 0;
    const lower = text.toLowerCase();
...
export function rerankCandidates(candidates, opts) {
...
export function rerankDocuments(docs, opts) {
    const asCandidates = docs.map((d) => ({ id: d.id, title: d.title, createdAt: d.createdAt, updatedAt: d.updatedAt, highlights: [docText(d).slice(0, 400)], trace: [`document rerank candidate from ${d.messages.length} messages`] }));
